# incubation period (only, not the joint distributions with Ct threshold 
# timings)
...
factor_order <- c("Delta", "Omicron (BA.1)", "Omicron (BA.2)", "Symptomatic", 
                  "3 exposures", "4 exposures", "5+ exposures", "Age: 20-34", 
                  "Age: 35-49", "Age: 50+")

dt_inc_period[, predictor := fct_relevel(predictor, factor_order)]

p_ip_dist <- dt_inc_period[!is.na(regressor_category) & predictor != "Asymptomatic"] %>% 
  ggplot() + 
...
# incubation periods
dt_inc_period[, `:=` (me = quantile(ip_draw, 0.5, na.rm = TRUE),
                      lo = quantile(ip_draw, 0.025, na.rm = TRUE), 
                      hi = quantile(ip_draw, 0.975, na.rm = TRUE)),
              by = c("predictor", "regressor_category")]

p_ip_eff <- dt_inc_period[!is.na(regressor_category) & predictor != "Asymptomatic"] %>% 
  ggplot() + 
...
ggsave("outputs/figures/pdfs/inc_periods.pdf",
       p_ip,
       width = 8,
       height = 8)
