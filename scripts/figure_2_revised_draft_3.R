...
p_fig_2_a <- figure_2_panel_a(dt_plot, flip_arg = FALSE) + 
...
p_fig_2_c_1 <- obs[, .SD[which.min(ct_value)], by = "id"
                   ][VOC == "Omicron (BA.1)", VOC := "BA.1"
                   ][VOC == "Omicron (BA.2)", VOC := "BA.2"] |> 
  unique() |> 
...
ggsave("outputs/figures/pngs/figure_2_new_draft_3.png",
       p_fig_2,
       width = 14,
       height = 10,
       bg = "white")

ggsave("outputs/figures/pdfs/figure_2_new_draft_3.pdf",
       p_fig_2,
       width = 14,
       height = 10,
       bg = "white")
