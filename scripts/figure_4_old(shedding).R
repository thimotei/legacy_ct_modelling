...
# dt_shedding_tmp <- dt_shedding_calc[, .SD[which.min(abs(t - inc_mean_nat))],
#                  by = c(".draw", "predictor", "regressor_category")]
# 
# dt_shedding_tmp[, .(me = quantile(auc_prop, 0.5),
#                     lo60 = quantile(auc_prop, 0.15),
#                     hi60 = quantile(auc_prop, 0.60),
#                     lo95 = quantile(auc_prop, 0.025),
#                     hi95 = quantile(auc_prop, 0.975)),
#                 by = c("predictor")] %>% 
#   ggplot() +
...
p1_fig4 <- figure_4_panel(shedding_draws_plot,
               ip_draws,
               "VOC",
               factor_arg = c("Delta",
                              "Omicron (BA.1)",
...
p2_fig4 <- figure_4_panel(shedding_plot_dt,
                          ip_draws,
                          "Symptom status",
...
p3_fig4 <- figure_4_panel(shedding_plot_dt,
                          ip_draws,
                          "Age",
...
p4_fig4 <- figure_4_panel(shedding_plot_dt,
                          ip_draws,
                          "Number of exposures",
...
fig4 <- plot_grid(p1_fig4, p2_fig4, p3_fig4, p4_fig4, ncol = 2)
...
# summarise shedding draws for plot
# "effect size" style estimates are too sensitive to the viral load conversion
# to get anything meaningful out of them...need to think if there is something
# clever we can do to ensure the estimates are meaningful
