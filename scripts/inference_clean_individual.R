...
#--- fitting for delta
stan_data_delta <- stan_data_fun(dt.delta)
...
dt.c.switch.delta <- res_delta$c_s_mean %>% 
  reshape2::melt() %>% 
  data.table() %>% 
  .[, smp := (mx - mn) * value + mn] %>%
  .[, voc := "Delta"]

dt.c.switch.omicron <- res_omicron$c_p_mean %>% 
  reshape2::melt() %>% 
  data.table() %>% 
  .[, smp := (mx - mn) * value + mn]  %>% 
  .[, voc := "Omicron"]
