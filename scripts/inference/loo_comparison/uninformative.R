# running the setup script
source("scripts/setup/main.R")

# this script has the same setup as the main fit, the uninformative priors
# are set from the epict() function call with the informative_priors boolean 

# seed used 
seed_eff <- 485764
set.seed(seed_eff)

# Fit the model
fit_uninformative <- epict(
  obs,
  ct_model = ct_model,
  adjustment_model = adjustment_model,
  likelihood = TRUE, # flag switching likelihood component of model on or off. Off means priors alone are sampled from
  onsets = TRUE, # include the symptom onset data and likelihood components
  switch = FALSE, # include the longer wane part of the model
  individual_variation = 0.025, # control the amount of 
  individual_correlation = 1.0,
  chains = 4,
  parallel_chains = 4,
  threads_per_chain = 4,
  iter_warmup = 1000,
  iter_sampling = 2000,
  adapt_delta = 0.95,
  max_treedepth = 12,
  output_loglik = TRUE,
  seed_manual = seed_eff,
  informative_priors = FALSE
)

# saves fitted object - usually results in a very large file (> 100Mbs)
# if the full model is fit
fit_uninformative$save_object("outputs/fits/uninformative.rds")
