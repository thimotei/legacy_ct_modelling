functions {
  vector piecewise_ct(vector t, real c0, real cp, real cs, real clod,
                      real te, real tp, real ts, real tlod, int switch) {
  ...
  vector piecewise_ct_by_id(vector t, real c0, vector cp, vector cs,
                            real clod, real te, vector tp, vector ts,
                            vector tlod, array[] int id,
                            array[] int tests_per_id,
    ...
    for (k in 1 : P) {
      ...
      ct[t_start : t_end] = piecewise_ct(t[t_start : t_end], c0, cp[k],
                                         cs[k], clod, te, tp[k], ts[k],
                                         tlod[k], switch);
    }
  ...
  vector onsets_lmpf(real inc_mean, real inc_sd, vector beta_im,
                     vector beta_is, matrix design, vector onset_avail,
                     vector onset_time, vector onset_window, vector inf_time,
                     array[] int ids) {
    ...
    for (i in 1 : nonsets) {
      tar[ids[i]] = log_diff_exp(lognormal_lcdf(onset_upper[i] | inc_mean_p[ids[i]], inc_sd_p[ids[i]]),
...
parameters {
  ...
  // Declaring adjustment parameters, adjusting the process parameters
  // for each covariate included in the design matrix passed to Stan from R
...
transformed parameters {
  ...
  
  {
    ...
    if (switch) {
      ...
      c_s = c_0
            * inv_logit(combine_effects(c_s_mean[1], beta_c_s, design)
                        + eta[ : , 5]);
      c_p = c_s .* c_p;
    } else {
  ...
  exp_ct = piecewise_ct_by_id(inf_rel, c_0, c_p, c_s, c_0, t_e, t_p, t_s,
                              t_lod, id, tests_per_id, cum_tests_per_id,
                              switch);
  ...
  if (any_onsets) {
    ...
    onsets_ttar = onsets_lmpf(inc_mean[1], inc_sd[1], beta_inc_mean,
                              beta_inc_sd, design, onset_avail, onset_time,
                              onset_window, t_inf, ids_with_onsets);
...
model {
  ...
  t_p_mean ~ normal(1.61, 5); //mean at log(5)
  t_lod_mean ~ normal(2.3, 5); //mean at log(10) + peak + switch timing 
  if (switch) {
    c_s_mean ~ normal(0, 5); //mean at 50% of maximum ct
    t_s_mean ~ normal(1.61, 5); //mean at log(5) + peak timing
  } 
  ...
  if (output_loglik) {
    ...
    for (i in 1 : N) {
      if (uncensored_by_test[i] == 1) {
        log_lik[i] = normal_lpdf(ct_value[i] | adj_exp_ct[i], sigma)
                     - normal_lccdf(0 | adj_exp_ct[i], sigma);
      } else {
        log_lik[i] = normal_lccdf(c_lod | adj_exp_ct[i], sigma)
                     - normal_lccdf(0 | adj_exp_ct[i], sigma);
      }
    }
  }
