...
subset_data_fun <- function(dt_clean_in, voc, no_pos_swabs) {
  
  dt_out <- dt_clean[VOC == voc] %>% 
    .[, t_first_test := as.numeric(swab_date - min(swab_date), units = "days"),
      by = c("id", "infection_id")] %>% 
    .[no_pos_results > no_pos_swabs] %>% 
    .[, id := .GRP, by = id] %>% 
    .[, c("id", "infection_id", "swab_date", "t_first_test", "ct_value", 
          "ct_value_std", "symptom_onset_date", "result", "pcr_res")]
